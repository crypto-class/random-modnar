...
int main()
{
	...
	cout << "---------C-------------" << endl;
	cout << "C don't know passwd, so just send C.S = 0" << endl;

	/************Attack*****************/
	SHA256 hash;
	client.S = 0;
	string ss = Inter2Str(client.S);
	cout << " CS = " << ss << endl;
	StringSource s2(ss, true, new HashFilter(hash, new HexEncoder(new StringSink(client.K))));
	cout << "CK = " << client.K << endl;
