...
class Send extends Thread{
	...
	private void atk(byte[] data){
		String str = "comment1=cooking%20MCs;userdata=1;comment2=%20like%20a%20pound%20of%20bacon";//已知原文
		int len = str.length();
		...
		str="admin=true;comment2=";//要修改成的字符串
		...
		for(int i = 0; i < fix.length; i++){
			 ...
			 * c[i] = c[i]^p1[i]^p2[i]
			 */
			data[len + i] = (byte) (data[len + i] ^ origin[len + i] ^ fix[i]);
		}
	}
